...
def live_price_inr(ticker):
    """
    Try live market quote via lightweight LTP endpoint first.
    If market is closed or blocked (returns 0), fall back to last historical close.
    """
    if ticker == "BTC-USD":
        try:
            r = requests.get(
    ...
    # Try live quote via UPSTOX LTP Endpoint
    ltp = None
    try:
        enc = quote(inst, safe="")
        r = requests.get(
            f"{UPSTOX_BASE}/v2/market-quote/ltp?instrument_key={enc}",
    ...
    # If live quote returned 0 or failed → market closed or blocked → use last close
    if not ltp or ltp == 0:
